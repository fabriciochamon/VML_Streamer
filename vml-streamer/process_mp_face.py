...
class MediaPipe_Faces:
	...
	def on_detection(self, result: vision.FaceLandmarkerResult, output_image: mp.Image, timestamp_ms: int):
		...
		for i, face_landmarks in enumerate(result.face_landmarks):
			...
			###################################################################################################
			'''
			model_points = np.float32([[-l.x, -l.y, -l.z] for l in face_world_landmarks])
			image_points = np.float32([[l.x * self.frame_width, l.y * self.frame_height] for l in face_landmarks])
			...
			for n, lm in enumerate(world_points):
				...
				for m in range(3):
					self.filtered_vals[f'{face_name}{n}{m}'] = lm[m]	

					if self.apply_filter:
						if f'{face_name}{n}{m}' not in self.mp_data_filter.keys():
							self.mp_data_filter[f'{face_name}{n}{m}'] = OneEuroFilter(self.iteration, lm[m], min_cutoff=self.one_euro_min_cutoff, beta=self.one_euro_beta)
							self.filtered_vals[f'{face_name}{n}{m}'] = lm[m]							
						else:
							try:
								self.filtered_vals[f'{face_name}{n}{m}'] = self.mp_data_filter[f'{face_name}{n}{m}'](self.iteration, lm[m])
								if np.isnan(self.filtered_vals[f'{face_name}{n}{m}']):
									self.mp_data_filter[f'{face_name}{n}{m}'] = OneEuroFilter(self.iteration, lm[m], min_cutoff=self.one_euro_min_cutoff, beta=self.one_euro_beta)
									self.filtered_vals[f'{face_name}{n}{m}'] = lm[m]	
							except:
								self.filtered_vals[f'{face_name}{n}{m}'] = lm[m]	
				...
				self.joints[face_name].append(
					{
						'x':self.filtered_vals[f'{face_name}{n}0'],
						'y':self.filtered_vals[f'{face_name}{n}1'],
						'z':self.filtered_vals[f'{face_name}{n}2'],
					}
				)
			'''
